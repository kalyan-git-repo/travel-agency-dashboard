...
const TripDetail = ({loaderData}:Route.ComponentProps) => {
    console.log(loaderData?.trip)
    const imageUrls = loaderData?.trip?.imageUrls || []
    const tripData = parseTripData(loaderData?.trip?.tripDetails)
    const {name, duration, itinerary, travelStyle, groupType, budget, interests, estimatedPrice, description,
        bestTimeToVisit, weatherInfo, country} = tripData || {}
            ...
            <section className="container wrapper-md">
                <header>
                    ...
                    <div className="flex items-center gap-5">
                        ...
                        <InfoPill text={itinerary?.slice(0,2).map((item) => item.location).
                        join(', ') || ''} image="/assets/icons/location-mark.svg" />
                    </div>
                </header>
                    ...
                    <ChipListComponent id="travel-chip">
                        <ChipsDirective>
                            {pillItems.map((pill,i) => (
                                <ChipDirective key={i} text={getFirstWord(pill.text)} cssClass={`${pill.bg} !text-base 
                                !font-medium !px-4`} />
                            ))}
                        </ChipsDirective>
                    </ChipListComponent>
            ...
            <section className="flex flex-col gap-6">
                ...
                <div className="trip-grid">
                    {allTrips.map(({id, name, imageUrls, itinerary, interests, travelStyle, estimatedPrice}) => (
                        <TripCard id={id} key={id} name={name} location={itinerary?.[0].location ?? ''} imageUrl={imageUrls[0]}
                                  tags={[interests, travelStyle]} price={estimatedPrice} />
                    ))}
                </div>
            </section>
        </main>
